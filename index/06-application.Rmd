...
getdata<- function(snp.name, iter=5000, drop= NULL){
  ...
  discovery.sites <- which(levels(factor(tp53epi.wsi$site))%in% drop)
...
p53.simnew = jags(data=p53.data , inits=NULL, parameters.to.save =parameters, model = p53.newmodel) #data aug?
...
getOR<-function(sim){
  ...
  sum.OR = t(apply(exp(OR), 2, function(x) {PI = HPDM(as.matrix(x))
  return(c(median(x),mean(x), PI[1], PI[2]))} #how to deal w multi, change HPD to have point mass at 1
  ))
  if (dim(sum.OR)[1]!= p53.data$n.sites+1){
    sum.OR1<- matrix(NA, p53.data$n.sites+1, dim(sum.OR)[2])
    sum.OR1[-p53.dataval$discovery.sites,]<- sum.OR
    sum.OR<- sum.OR1
  } ###
  colnames(sum.OR)<-c("Median","Mean", "2.5%","97.5%")
  return(sum.OR)
}
...
kable(ORtable)
simlist<- list(p53.sim,p53.simval,p53.simnew,p53.simnormal,p53.bfsim)
modelnames<- c("Original","Validation", "Bayesian", "Cond", "BF")
plots<-function(simlist){
  ORlist = lapply(simlist, function(sim){ 
    OR<-as.matrix(as.data.frame(sim$BUGSoutput$sims.matrix)%>%select( starts_with("beta.p53"), "mu.p53"))
    if (dim(OR)[2]!= p53.data$n.sites+1){
      n = dim(OR)[1]
      OR1<- matrix(NA, n, p53.data$n.sites+1) ##!!!!
      val.sites<- setdiff(1:(p53.data$n.sites+1), p53.dataval$discovery.sites)
      OR1[,val.sites]<- OR
      OR<- OR1
    }
    OR.df<- as.data.frame(OR)
    colnames(OR.df)<- rownames(sum.OR1) ##!!!!
    return(exp(OR.df))
  }
  )
  plotdat<-NULL
  for(i in 1:length(ORlist)){
    plotdat<- rbind(plotdat, data.frame(melt(ORlist[[i]]), model = modelnames[i]))
  }
  
  plt<-ggplot()+geom_boxplot(data = plotdat, outlier.shape = NA,aes(x=variable, y=value, fill=factor(model)))
...
```{r all together}
getalldata<- function(iter=5000, drop= NULL){
