...
get.cond.likelihood.data<- function(data, inits, p = 0.00325){
...
run.all<- function(assoc,mu, sd, inits){
  newdata = NULL;count=0
  while(is.null(newdata) & count<1000){
    data <- sample(assoc, mu, sd)
    newdata<-get.cond.likelihood.data(data$simdata)
    count=count+1
  }
  #run jags
...
make_plots<-function(sim, mu, sd){
  ...
  coverage1$value<-factor(levelnames[coverage1$value])
