...
knitr::opts_chunk$set(message = FALSE, warning = FALSE, cache=TRUE,echo=FALSE,  fig.width=6, fig.height = 4, out.width='6in' ,fig.align='center', dpi = 600)
...
g <- ggplot(data = data.frame(maxy = dnorm(.4, .4,.2)/.6, x = mu, y = dnorm(mu, .4,.2), xl=0, xu=0,yl=0,yu=.4))+
...
ggplot(melted, aes(mu, value, color = variable)) + geom_line(aes(group=variable)) +
  labs(x = expression(mu), y = "Likelihood")+  theme_classic()+ 
...
ggplot(data=melt(BFapproxdata,id.vars = "xi"), aes(y=xi))+
  ...
  labs(x = expression(xi), y = "Density")+  theme_classic()+ 
  #theme(legend.justification=c(1,1), legend.position=c(1,1)) +
...
g<-ggplot(data = melt(estimators), aes(x=variable, y=value)) + geom_boxplot()+labs(x= "Estimator", y="|Bias|")+theme_classic() + scale_colour_few()
...
<<>>=
ggplot(data = results)+ geom_point(aes(x = conf.upper-conf.lower, y = cred.upper-cred.lower, colour = abs(Ybar)))+geom_abline(intercept = 0, slope = 1)+geom_hline(yintercept=2*1.96*sqrt(1/100))+geom_vline(xintercept=2*1.96*sqrt(1/100))+labs(x="Length of (Conditional) Credible Interval",y="Length of (Bayesian) Credible Interval", color="|Ybar|")+theme_classic() 
...
mseplot = ggplot(data = (df1), aes(x=factor(simulation, labels=c(0,"s/2", "s", "2s", "4s")), y=value)) +
  ...
  labs(x=expression(paste(sigma^2, " used in simulation")),y="Absolute Error")+
...
pindmed<- cbind(sapply(c(1,3,4), function(x) sqrt(mean((sim.0[14,x,])^2))),
                sapply(simslist, function(sim) 
                  sapply(c(1,3,4), function(x) sqrt(mean((sim[14,x,])^2)))))
rownames(pindmed)<-modelnames[c(1,3,4)]
colnames(pindmed)<-c("mu=0","s/2", "s", "2s", "4s")

zeromu<- cbind(sapply(c(1,3,4), function(x) sqrt(mean((sim.0[7,x,])^2))),
               sapply(simslist, function(sim) 
                 sapply(c(1,3,4), function(x) sqrt(mean((sim[7,x,])^2)))))
rownames(zeromu)<-modelnames[c(1,3,4)]
...
intlenplot = ggplot(data = (df), aes(x=factor(simulation, labels=c(0,"s/2", "s", "2s", "4s")), y=value)) +
  geom_boxplot(aes(color=Var2))+  theme_classic() +
  labs(x=expression(paste(sigma^2, " used in simulation")),y="Credible Interval Length")+
  scale_colour_few(name="Model") #+  theme( legend.justification=c(0,1), legend.position=c(0,1))
...
getOR<-function(sim, totalsites,discoverysites,multimodal=TRUE){
  
  OR = as.data.frame(sim$BUGSoutput$sims.matrix)%>%select( starts_with("beta.p53"), "mu.p53")
  exp(OR)
  #colnames(OR) = c(site.names, "Overall")
  
  sum.OR = t(apply(exp(OR), 2, function(x) {
    if(multimodal){
      PI = HPDM(as.matrix(x),pointmass=1)
  return(c(median(x), PI[1], PI[2], PI[3],PI[4]))
    }
    else{
      PI = HPD(as.matrix(x))
      return(c(median(x), PI[1], PI[2], NA, NA))

    }
    } #how to deal w multi, change HPD to have point mass at 1
  ))
...
ORbySNP<- lapply(allSNP, function(snplist)
  lapply(1:(length(snplist)-2),function(x)
    getOR(snplist[[x]],snplist[[(length(snplist)-1)]],snplist[[(length(snplist))]], (x!=1&x!=3))))
...
save(ORbySNP, snps, H1bySNP, file="p53OR.RData")
@
...
overall$model = rep(c("HB","HSS","CL","AHSS"),length(snps))
overall$model<- factor(overall$model, levels= c("HSS","CL","AHSS","HB"))
...
overall$pointmass<-NA
overall$pointmass[which(overall$U-overall$L==0 & !is.na(overall$L1))]<-1

ggplot(data = overall, aes(group=model,color=model,x = snp,xend = snp))+ 
  geom_hline(yintercept=1, alpha=.3)+
  geom_linerange(aes(ymin=L, ymax=U), position = position_dodge(width = .7))+
  geom_linerange(aes(ymin=L1, ymax=U1), position = position_dodge(width = .7))+ 
  geom_point(aes(x=snp, y=X,shape=model),size=2, position = position_dodge(width = .7)) +
  geom_point(aes(y=pointmass),size=1,position = position_dodge(width = .7))+
  theme_classic() + scale_colour_few()+labs(y="Odds Ratio")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

@
\end{figure}

<<>>=
colnames(H1bySNP)<-snps
rownames(H1bySNP)<-c("HB","HSS","CL","AHSS")
kable(H1bySNP,caption = "Proportion of H1 in Posterior Samples", digits = 3)
@
...
The full tables of OR by site and SNP can be found in the supplement.
...
The CL and AHSS methods can be used in follow-up studies even when the discovery data is not publicly available. Both provide significant improvements over the naive method. The AHSS model has a quasi-testing feature since it accounts for the probability of a true association, but is very sensitive to the p-value as well as the choice of prior, and can be illogical for a prior such as the uniform. However, it is extremely fast and requires very little data, making it easy to use.
...
for(i in 1:length(snps)){
  print(kable_styling(kable(makeORtable(ORbySNP[[i]], all.sitenames[[i]]), #caption=paste0("\\label{tab:", snps[i],"}"))
    caption = snps[i]),latex_options = "scale_down"))
}
